...
class sphere : public hittable {
    ...
    // moving sphere
    sphere(const vec3& center1, const vec3& center2, double radius, shared_ptr<material> mat) : center(center1, center2 - center1), radius(radius), mat(std::move(mat)) {
    }

    bool hit(const ray& r, interval ray_t, hit_record& rec) const override {
        vec3 current_center = center.at(center.time());
